...
class Slides(composer.Task):
  ...
  def __init__(self,
    ...
    self._walker_spawn_position = distributions.Uniform(
        low=np.array(1.0), high=np.array(1.5))
      # low=np.array(5.0), high=np.array(5.5))
    
    self._slides_slope = None # todo
    # vertices 0.3 5 0  0.3 -5 0  5.3 5 1.75  5.3 -5 1.75  10.3 5 0  10.3 -5 0
    # import file with 
    FILE = os.path.join(os.path.dirname(__file__), 'assets', 'slide.stl')
    mesh = self.root_entity.mjcf_model.asset.add('mesh', file=FILE, name='slide', scale=[0.2, 0.30, 0.05])
    model = self.root_entity.mjcf_model
    num_slides = 10; gap_between_slides = 5.0
    for i in range(0, num_slides):
        model.worldbody.add('geom', type='mesh', mesh='slide', name=f'slide{i}', 
                            friction=[0.6, 0.005, 0.0001],
                            pos=[(i*gap_between_slides), -6.02, 0], rgba=[1., 0.6, 0.6, 1])
    enabled_observables = []
