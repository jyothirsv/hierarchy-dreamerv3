...
class PolesCorridor(EmptyCorridor):
    ...
    def _build(self, pole_radius=0.05, pole_height=2.0, pole_spacing=2.0, pole_position=0.0,
        ...
        self._pole_spacing = pole_spacing
        self._pole_position = pole_position
        self._include_initial_padding = include_initial_padding
        self.mjcf_model.worldbody.add("camera", name="global", mode="fixed", pos=[0, 0, 15], zaxis=[0, 0, 1]
                                              , fovy=90)
    ...
    def regenerate(self, random_state):
        ...
        pole_spacing = variation.evaluate(self._pole_spacing, random_state=random_state,)
        ...
        for i in range(1, rows + 1):
            ...
            # # y_even = [4.5, 1.5, -1.5, -4.5]
            # # y_odd = [3, 0, -3]
            # # include pole_spacing in y_even and y_odd
            # y_even = [3,1,-1,-3]
            # y_odd = [ 2 , 0, -2]
            # if i % 2 == 0:
            #     positions = [(x_position, y*pole_spacing/2, 0) for y in y_even]
            # else:
            #     positions = [(x_position, y*pole_spacing/2, 0) for y in y_odd]
            # print(positions)
            # Add poles to the model worldbody
            # sample three numrandomly from -5 to 5
            positions = [(x_position, variation.evaluate(self._pole_position,random_state), 0) for _ in range(3)]
...
class SlidesCorridor(EmptyCorridor):
    ...
    def _build(self, slide_length=5.0, slide_height=0.05, slide_spacing=2.0,
        ...
        # self.slide_mesh = self.mjcf_model.asset.add('mesh', file=file_path, name='slide_mesh', scale=[0.2, 0.26, 0.05])
        self.mjcf_model.worldbody.add("camera", name="global", mode="fixed", pos=[0, 0, 15], zaxis=[0, 0, 1]
                                              , fovy=90)

    def add_scaled_mesh(self, id, scale):
        """Helper function to create a scaled mesh asset."""
        file_path = os.path.join(os.path.dirname(__file__), 'slide.stl')
        return self.mjcf_model.asset.add('mesh', file=file_path, name=f'slide_mesh_{id}', scale=scale)
    ...
    def regenerate(self, random_state):
        ...
        while slide_x < self._corridor_length:
            print(f"{id=} {slide_x=} {slide_length=} {slide_height=}")
            id += 1
            slide_pos = [slide_x, -5.3, 0]

            self.slide_mesh = self.add_scaled_mesh(id, scale=[0.2, 0.26, slide_height])
            self.mjcf_model.worldbody.add('geom',
                                          type='mesh',
                                          mesh='slide_mesh_{}'.format(id),
                                          name='slide_geom_{}'.format(id),
                                          pos=slide_pos,
            ...
            slide_length = variation.evaluate(self._slide_length, random_state=random_state)  # New length for next slide
            slide_height = variation.evaluate(self._slide_height, random_state=random_state)
